...
sub palettePreset {
  ...
  ## 魔物
    ...
    $text .= "$::pc{vitResist}（<f>$::pc{vitResistFix}+{生命抵抗修正}</f>） 生命抵抗力\n" if $achievementFixedEnabled;
    $text .= "2d+{精神抵抗}+{精神抵抗修正} 精神抵抗力\n" if $achievementDiceEnabled;
    $text .= "$::pc{mndResist}（<f>$::pc{mndResistFix}+{精神抵抗修正}</f>） 精神抵抗力\n" if $achievementFixedEnabled;
    $text .= "\n";
    if ($::pc{statusNum} > 1) {
      foreach (1 .. $::pc{statusNum}) {
    ...
    foreach (1 .. $::pc{statusNum}){
      ...
      if ($::pc{statusNum} > 1 && $::pc{'status'.$num.'Evasion'} ne '') {
        ...
        $text .= "回避${part} {回避$_}（<f>" . ($::pc{'status'.$num.'Evasion'} + 7) . "+{${partName}_回避修正}+{回避修正}</f>）\n" if $achievementFixedEnabled;
    ...
    $skills =~ s/^
      ...
      (?<power>[0-9]+)
      (?:[(（][0-9]+[）)])?
      /$text .= ($achievementDiceEnabled ? "2d+{$+{name}} $+{name}\n" : '') . ($achievementFixedEnabled ? "$+{power}（<f>" . ($+{power} + 7) . "<\/f>） $+{name}\n" : '') . "\n";/megix;
    ...
    $skills =~ s/^
      ...
      (?=^$skill_mark|^●|\z)
      /
      $text .= ($achievementFixedEnabled || $+{base} eq '' ? (convertMark($+{mark})."$+{name}".($+{fix} ne '' || $+{other} ne '' ? "／$+{fix}$+{other}" : '')."\n") : '')
            .($+{base} ne '' && $achievementDiceEnabled ?"2d+{$+{name}} ".convertMark($+{mark})."$+{name}$+{other}\n":'')
