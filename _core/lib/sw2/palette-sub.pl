  ...
  if(!$type){
    ...
    foreach my $class (@classNames){
      ...
      foreach my $p_id (sort{$data{$a}{stt} cmp $data{$b}{stt} || $data{$a} cmp $data{$b}} keys %data){
        ...
        my $initiativeModifiers = makeStatesExpression(\%::pc, ['先制判定', '敏捷度ボーナス']);
    ...
    foreach my $class (@classNames){
      ...
      foreach my $paNum (0 .. $::pc{paletteMagicNum}){
        ...
        elsif($name =~ /歌/){ $text .= "${modification}+{行為判定修正}+{行動判定修正} @{[$::SW2_0 ? '呪歌演奏' : '演奏判定']}\n"; }
    ...
    if ($::pc{individualization}) {
      if ($::pc{mount}) {
        ...
        foreach (1 .. $::pc{statusNum}) {
          ...
          $::pc{'status' . $num . 'Defense'} += $::pc{'partEquipment' . $_ . '-armor-defense'} if $::pc{'status' . $num . 'Defense'} ne '';
