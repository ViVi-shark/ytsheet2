...
print <<"HTML";
            <tbody id="weapon-row$num">
              <tr>
                ...
                <td><select name="weapon${num}Category" oninput="calcWeapon()">@{[option("weapon${num}Category",@data::weapon_names,'ガン（物理）','ガン（近接）','その他|<その他（盾など）>')]}</select>
              ...
              <tr class="note">
                <td colspan="9">@{[textarea("weapon${num}Note",'calcWeapon','onchange="changeEquipMod();calcCash();changeWeaponName()" placeholder="備考"')]}
              <tr class="options">
                <td colspan="9">
