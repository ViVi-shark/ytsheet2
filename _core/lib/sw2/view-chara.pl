...
if($pc{ver}){
  foreach (keys %pc) {
    ...
    if ($_ =~ /^(?:items|freeNote|freeHistory|cashbook)$/) {
      ...
      $pc{$_} =~ s#(</(?:h[1-6]|p|section|details|summary|div)>)#$1\n#gi;
    }
...
if(!$pc{modeDownload}){
  ...
  if($::in{url}){
    ...
    if($pc{logId}){
      ...
      else                 { push(@menu, { TEXT => '復元', TYPE => "href"   , VALUE => "./?mode=edit&id=$::in{id}&log=$pc{logId}" . ($::in{f} ? '&selected_section=fellow' : ''), }); }
    }
    else {
      ...
      else                 { push(@menu, { TEXT => '編集', TYPE => "href"   , VALUE => "./?mode=edit&id=$::in{id}" . ($::in{f} ? '&selected_section=fellow' : ''), }); }
    }
  }
}
