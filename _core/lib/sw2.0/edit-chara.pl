...
HTML
  ...
  foreach my $lv (1..$c_max){
    print '<li id="craft-'.$name.$lv.'"><div class="select-input"><select name="craft'.$Name.$lv.'" oninput="'.($class =~ /アルケミスト/ ? 'updateAlchemyIcon(this);' : '').'checkCraft();'.$functions.'selectInputCheck(this);">';
    print '<option></option>';
